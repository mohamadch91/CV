...
\newcommand\normal{\@setfontsize\Large\@xviipt{14}}
...
\newcommand{\drawskillbars}[1]{
    \begin{tikzpicture}
        ...
        \foreach \i in {0, 1, 2, 3, 4}{
            \fill[lightgray] (\i * 0.4 + 0.2 *\i,0) rectangle (0.4 + \i * 0.4 + \i * 0.2,0.1);
        }
        
        % Draw number of black bars depending on the skill level
        \foreach \i in {#1}{
            \fill[black] (\i * 0.4 + 0.2 *\i,0) rectangle (0.4 + \i * 0.4 + \i * 0.2,0.1);
        }
    \end{tikzpicture} \par
}
...
\newcommand{\education}[4]{
    ...
    % School Name 
    \large
    \noindent \color{subsd} \parbox{.7\linewidth}{\textsl{#3}} \par
    % Description
